...
export async function chatWithLegacyStream(transcriptContext: string, question: string, history: {role: string, text: string}[] = [], linguisticContext?: string, relationalContext?: string, systemOverrides?: string): Promise<ReadableStream> {
  ...
  const prompt = `
    ...
    CRITICAL INSTRUCTIONS FOR YOUR RESPONSE:
    - STRICT SCOPE: Directly answer the user's specific question. Do not bleed into loosely related topics (e.g., if asked about biological children, do not write paragraphs about wives or step-children unless explicitly asked). Maintain absolute focus.
    - FORMATTING EXCELLENCE: Break down your answer into highly specific, granular headers. For example, if discussing multiple people, give each person their own distinct bolded header rather than grouping them into massive paragraphs.
    - Structure your response using well-defined, bolded thematic section headers. Do not just output a brief bulleted list. Write engaging, cohesive, multi-sentence paragraphs under each header.
    - SEAMLESS INTEGRATION: Do not narrate your instructions or thought process. Never use phrases like "per user directives", "according to the system overrides", or "based on the provided context". Simply integrate the facts seamlessly as if you natively knew them.
    ...
    ${systemOverrides ? `- ABSOLUTE SYSTEM OVERRIDES: The following corrections are ABSOLUTE TRUTHS. You MUST obey them over the retrieved context. However, you MUST integrate these truths silently. DO NOT narrate that you are making a correction or mention "user directives".: ${systemOverrides}` : ''}
